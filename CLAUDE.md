...
working CMS (image replace/remove + size/fit, text editing, brand/hero/product/
team/collection create, product tags, theme editor). By default it is
**unauthenticated** — the URL is the secret
...
- Migrations in `supabase/migrations/` are applied **in order, by hand** via the
  Supabase SQL Editor (0001 → 0010). There are ten; 0004 adds `site_content`,
  ...
  it's applied). 0003 is reseed-safe:
