...
interface Props {
  // Live catalog passed in by the server page (falls back to the compiled-in
  // PRODUCTS only when Supabase is unreachable). Drives the manual
  // "Add Product" picker, so dashboard-created products are offered and
  // soft-deleted ones aren't.
  products: Product[]
}

export default function MyQuoteClient({ products }: Props) {
  const { 
    items, 
    contactInfo, 
    projectInfo, 
